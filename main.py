...
#   p2_weights = randWeights()
  ...
  while board.running:
    ...
    if board.state == 'p1_turn':
      ...
      states_collected[board.to_play] = board.monte_carlo_turn(standard_weights, 1, num_sims=1500)
    ...
    if board.finished == [True,True]:
      ...
      print(len(states_collected), len(states_collected[0]), len(states_collected[1]), len(data_dict))
